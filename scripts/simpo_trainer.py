...
class SimPOTrainer(Trainer):
    ...
    def simpo_loss(
        ...
        """
        if self.loss_type == "exo":
            # learning rate, gamma, -10
            est_rewards = torch.stack([policy_chosen_logps - self.gamma_beta_ratio, policy_rejected_logps], dim=0).to(
                self.accelerator.device)
            est_rewards = self.beta * est_rewards
            log_est_rewards = est_rewards.log_softmax(dim=0)  # [2, B]
            energy_labels = torch.zeros_like(log_est_rewards)  # [2, B]
            energy_labels[0, :] = 0
            energy_labels[1, :] = -10
            losses = log_est_rewards.exp() * (log_est_rewards - energy_labels).sum(dim=0)  # [B]
        elif self.loss_type == "alphapo":
            alpha = self.alpha
            est_rewards = torch.stack([policy_chosen_logps - self.gamma_beta_ratio, policy_rejected_logps], dim=0).to(
                self.accelerator.device)
            est_rewards = self.beta * est_rewards
            ...
            energy_labels[0, :] = 0
            energy_labels[1, :] = -10
            u = (log_est_rewards - energy_labels).exp()
            losses = ((1.0 / (alpha * (alpha - 1.0))) * energy_labels.exp() * \
                      (u.pow(1.0 - alpha) - (1.0 - alpha) * u - alpha)).sum(dim=0)
        else:
